...
class HEDGE:
        ...
        for home in range(self.n_homes):
            if "EV" in self.data_types:
                ...
                if prev_clusters["EV"][home] == self.n_all_clusters_ev - 1:
                    ...
                    print(f"len(probabilities) EV {len(probabilities)}")
    ...
    def _adjust_max_ev_loads(self, day, interval_f_ev, factors,
        ...
        for home in range(self.n_homes):
            ...
            while np.max(day["loads_EV"][home]) > self.bat["c_max"] \
                    and it < 100:
                if it == 99:
                    print("np.max(loads_EV[home]) > self.max_load_ev[home]")
    ...
    def _load_ev_profiles(self, input_dir, profiles, prm):
        ...
        for data, label in zip(["cons", "avail"], ["norm_EV", "EV_avail"]):
            ...
            for file in files:
                if file[0] != ".":
                    ...
                    prof = np.shape(profiles["EV"][data][data_type][cluster])
                    if len(prof) == 1:
        ...
        for day_type in labels_day:
            ...
            print(f"self.n_prof['EV'][day_type] {self.n_prof['EV'][day_type]}")
        print(f"self.n_all_clusters_ev {self.n_all_clusters_ev}")
    ...
    def _load_dem_profiles(self, paths, profiles, prm):
        ...
        path = paths["save_path"] / "profiles" / "norm_dem"
        for day_type in prm["weekday_type"]:
    ...
    def _load_profiles(self, paths: dict, prm: dict) -> dict:
        ...
        paths['profiles_path'] = paths["save_path"] / "profiles"
        self.n_prof: dict = {}

        # EV profiles
        if "EV" in self.data_types:
            profiles \
                = self._load_ev_profiles(paths["save_path"], profiles, prm)
        ...
        if "gen" in self.data_types:
            profiles = self._load_gen_profiles(paths["save_path"], profiles)
