...
def save_outs(outs, prm, data_type, save_path):
    ...
    all_data = np.zeros((24, 366)) if data_type == "EV" else None
    granularities = []
    for out in outs:
        [days_, all_abs_error, types_replaced,
         all_data, granularities] \
    ...
    if prm["do_test_filling_in"]:
        print(f"len(all_abs_error) {len(all_abs_error)}")
        print(f"stats_filling_in {data_type}")
        stats_filling_in(
            prm, data_type, all_abs_error, types_replaced, save_path)

    print(f"len all days {len(days_)}")

    return days_


def remove_incomplete_days(days: list, to_throw: list) -> list:
    ...
    for day in days:
        assert len(day["mins"]) == 24, \
            f"error len(day['mins']) = {len(day['mins'])}"

    return days


def get_days_clnr(prm: dict, sequences: dict, data_type: str) \
    ...
    for id_, sequence in sequences.items():
        ...
        for t, cum_day in enumerate(sequence["cum_day"]):
            if t > 0 and cum_day != sequence["cum_day"][t - 1]:
                ...
                assert len(current_day["mins"]) <= 24, \
    ...
    days, to_throw, abs_error, types_replaced_eval = fill_whole_days(
        prm, days, data_type, sequences
    )

    del sequences
    days = remove_incomplete_days(days, to_throw)
...
def normalise(
    ...
    for i, day in enumerate(days):
        if day[data_type] is not None:
            ...
            if len(day["norm_" + data_type]) != n_time_steps:
                print(
...
def filter_validity(
    ...
    # range of dates
    print(
        f"data_type {data_type} data['start_avail'].min() = "
        f"{data['start_avail'].min()} (number of days since 01/01/2010)"
    )
    print(
        f"data_type {data_type} data['end_avail'].max() = "
        f"{data['end_avail'].max()} (number of days since 01/01/2010)"
    )

    data = obtain_time(data, data_source)

    print(f"len(list(data['weekday'])) {len(list(data['weekday']))}")
    data["keep"] = keep_column(
        data["cum_day"], data["start_avail"], data["end_avail"]
    )
    print(f"len(list(data[data.keep]['weekday'])) {len(list(data[data.keep]['weekday']))}")
...
def import_segment(
    ...
    print(f"start import_segment {data_type}")
    data_source = prm["data_type_source"][data_type]
    ...
    return days, abs_error, types_replaced_eval, all_data, granularities
...
def get_data(
    ...
    # 1 - get raw data
    # skiprows with + 1 to remove header row with string instead of numbers
    #
    # with open(paths["var_path"][data_type], "rt") as file:
    #     reader = csv.reader(
    #         file,
    ...
    #     data = pd.DataFrame(
    #         [
    #             [reader[i][col] for col in prm["i_cols"][data_type].values()]
    #             for i in block_indexes
    #         ],
    #     columns=list(prm["i_cols"][data_type])
    #     )
    # data = pd.DataFrame(columns=list(prm["i_cols"][data_type]))
    # rows = []

    # for i in block_indexes:
    #     rows.append(pd.read_csv(
    ...
    if data_type == "EV":
        for col in data.columns:
            data_array = [data[col].iloc[i] for i in range(5)]
            print(f"EV {col}: {data_array}")

    # 2 - filter data
    data = filter_validity(
